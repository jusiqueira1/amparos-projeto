...
document.addEventListener('DOMContentLoaded', async () => {
    const usuariaId = localStorage.getItem('id_usuaria');

    if (!usuariaId) {
        return alert('ID da usuária não encontrado. Por favor, faça login novamente.');
    }

    try {
        const response = await fetch('http://localhost:3004/api/get/perfil/usuaria', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: usuariaId })
        });
        const result = await response.json();

        if (result.success) {
            const usuariaInfo = document.getElementById('usuariaInfo');
            usuariaInfo.innerHTML = `
                <p><strong>Nome:</strong> ${result.data.nome}</p>
                <p><strong>Idade:</strong> ${result.data.idade}</p>
        ...
        console.error("Erro ao buscar perfil da usuária:", error);
    }
});
...
document.getElementById('editarButton').addEventListener('click', () => {
    ...
    // Captura os valores do perfil exibido
    const nome = document.querySelector('#usuariaInfo p:nth-child(1)').textContent.split(': ')[1];
    const idade = document.querySelector('#usuariaInfo p:nth-child(2)').textContent.split(': ')[1];
    const email = document.querySelector('#usuariaInfo p:nth-child(3)').textContent.split(': ')[1];
...
document.getElementById('saveButton').addEventListener('click', async () => {
    ...
    if (!usuariaId) {
        return Swal.fire({
            title: 'Erro!',
            text: 'ID da usuária não encontrado. Por favor, faça login novamente.',
            icon: 'error',
            confirmButtonText: 'OK'
        });
    }
    ...
    try {
        const response = await fetch('http://localhost:3004/api/edit/perfil/usuaria', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id_usuaria: usuariaId, nome, idade, email })
        });

        const result = await response.json();

        if (result.success) {
            document.getElementById('usuariaInfo').innerHTML = `
